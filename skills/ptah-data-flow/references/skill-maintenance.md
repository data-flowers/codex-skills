...
5. Run `python3 scripts/check_skill.py` from the skill directory for the offline behavioral suite, Python/JavaScript syntax checks, and reference-link checks. Also run the skill frontmatter validator and repository whitespace check. Tests must make no live model or Airtable requests.
