...
- Treat direct SVG and ICO attachments as unsupported. Download and convert them locally before any Airtable upload or URL-based attachment PATCH: rasterize SVG at sufficient resolution, select the largest embedded ICO frame, and publish the resulting validated PNG or WebP instead of the original source URL. The bundled attachment optimizer performs this normalization to WebP during its prepare phase.
